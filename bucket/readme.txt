...
!!! Возникает проблема с отображэением списка товаров, т.к. нужно к примеруц еще грузить изображения.. Тут всех лучше справится s_list_class - поэтому форму эту можно переделать на макет.. 
3.1.1 
Шаблон навигации
<?php
$$b = new pBucket();
...
Шапка - вывод списка товаров
<?
$ids = array_keys($b->get());
echo s_list_class(6,8,'nc_ctpl=2008&id='.implode(',',$ids));
?>


В самом компоненте, в системных настройках:
<?
$ignore_sub = 1;
$ignore_cc = 1;
...
В этом случае форму отправки товара нужно делать в разделе Корзина. т.е. сам раздел идет со списокм товаров, и + комопнент коризны. 
